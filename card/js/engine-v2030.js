...
export function applyPremiumBranding(dbData = {}) {
    ...
    styleEl.textContent = `
        ...
        .social-icons-bar .social-icon[aria-label="Threads"] { background: #000000 !important; }
...
class SuitoGallery {
    constructor(galleryData, catalogUrl) {
    ...
    initLightboxDOM() {
        ...
        lightbox.innerHTML = `
            ...
            <div class="suito-lb-content">
                ...
                <div class="suito-lb-catalog" style="display:none;"></div>
    ...
    updateLightboxContent() {
        ...
        const catalogEl = lightbox.querySelector('.suito-lb-catalog');
        ...
        if (catalogEl) {
            if (this.catalogUrl) {
                ...
                catalogEl.innerHTML = `
                    <a href="${this.catalogUrl}" target="_blank" rel="noopener noreferrer"
                       style="display:flex;align-items:center;justify-content:center;gap:8px;margin-top:16px;padding:12px 24px;background:#25D366;color:#fff;font-weight:700;font-size:14px;border-radius:18px;text-decoration:none;box-shadow:0 4px 16px rgba(37,211,102,0.35);">
...
function attachCardEvents(container, data) {
    ...
    const catalogUrl = data.whatsapp_catalog_url || '';
    const galleryRoot = container.querySelector('.suito-gallery-mount');
    if (galleryRoot) {
        new SuitoGallery(galleryData, catalogUrl).renderGallerySection(galleryRoot);
    }
}
